...
public class OAuth2Util {
	...
	public static String generateBasicAuthHeaderString(RequestBaseVO token) {
		try {
			String base = "";
			if (token.getClient_secret() == null || token.getClient_secret() == "") {
				base = encodeURIComponent(token.getClient_id());
			} else {
				base = encodeURIComponent(token.getClient_id()) + ":" + encodeURIComponent(token.getClient_secret());
			}
	...
	public static void parseBasicAuthHeader(String authHeader, RequestBaseVO token) throws OAuth2Exception {
		try {
			// Basic XXXXXXXXX
